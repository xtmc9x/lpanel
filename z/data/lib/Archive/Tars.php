...
class Archive_Tars implements Archive_Interface
{
    ...
    public function listArchive ($down = '')
    {
        ...
        if (!$list) {
            ...
            return '<div class="list1"><td colspan="' . (array_sum(Config::getSection('Display')) + 1) . '">' . Helper_View::message(Language::get('archive_error'), Helper_View::MESSAGE_ERROR_EMAIL) . '</td></div>';
        } else {
            ...
            for ($i = 0; $i < $s; ++$i) {
                ...
                if ($list[$i]['typeflag']) {
                    ...
                    $down = '<a href="change.php?get=' . $r_current . '&amp;f=' . $r_name . '">[D]</a> ';
                }
                $l .= '<div class="list1"><td class="check"> <input name="check[]" type="checkbox" value="' . $r_name . '"/></td>';
                ...
                if (Config::get('Display', 'change')) {
                    $l .= '<a href="change.php?c=' . $r_current . '&amp;f=' . $r_name . '">[R]</a>';
                }
                if (Config::get('Display', 'del')) {
                    $l .= '<a onclick="return Gmanager.delNotify();" href="change.php?go=del_tar_archive&amp;c=' . $r_current . '&amp;f=' . $r_name . '">[E]</a>';
                }
