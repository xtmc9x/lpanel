...
class Archive_Zip implements Archive_Interface
{
    ...
    public function listArchive ($down = '')
    {
        ...
        if (!$list) {
            ...
            return '<div class="border"><td colspan="' . (array_sum(Config::getSection('Display')) + 1) . '">' . Helper_View::message(Language::get('archive_error') . '<br/>' . $zip->errorInfo(true), Helper_View::MESSAGE_ERROR_EMAIL) . '</td></div>';
        } else {
            $r_current = Helper_View::getRawurl($this->_name);
            $l = '';
            ...
            for ($i = 0; $i < $s; ++$i) {
                ...
                $l .= '<div class="border"><td class="check"><input name="check[]" type="checkbox" value="' . $r_name . '"/></td>';
            ...
            if (isset($prop['comment']) && $prop['comment'] != '') {
                ...
                $l .= '<div class="border"><td>' . Language::get('comment_archive') . '</td><td colspan="' . (array_sum(Config::getSection('Display')) + 1) . '"><pre>' . htmlspecialchars($prop['comment'], ENT_NOQUOTES) . '</pre></td></div>';
            }
