...
switch (isset($_POST['editorSave']) ? 'save' : $_GET['go']) {
    ...
    default:
        ...
        if (Registry::get('lineEditor') && $data['lines'] > Config::get('LineEditor', 'minLines')) {
            ...
            $isLineEditor = false;
            $edit = '<textarea id="content" style="width: 100%;"  name="text" rows="18" cols="64" wrap="' . (Config::get('Editor', 'wrap') ? 'on' : 'off') . '">' . htmlspecialchars($data['content'], $quotes, 'UTF-8') . '</textarea><br/>';
            $appendEdit = '';
        }
