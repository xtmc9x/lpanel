...
&nbsp;&#183;<a href='$url&q=show+processlist'>Tác vụ</a>";
 ...
 <input type='submit' value='Xóa' onclick=\"if(ays()){sht('drop')}else{return false}\">
...
<input type="button" name="Clear" value="Xóa hết " onclick="document.DF.q.value=''" style="width:100px">
</div>
...
<div class="frm">
<label class="l">Tài khoản:</label><input type="text" name="v[user]" value="<?php eo($DB['user'])?>"><br>
...
<label class="l">Tên CSDL:</label><input type="text" name="v[db]" value="<?php eo($DB['db'])?>"><br>
...
<input type="submit" value="áp dụng "><input type="button" value="Hủy" onclick="window.location='<?php eo($self)?>'">
</center>
</div>
</center>
<?php
...
function db_connect($nodie=0){
 ...
 if ($dbh && $DB['db']) {
  ...
  if (!$res) {
     $err_msg='Chọn không dfược CSDL: '.mysql_error();
     if (!$nodie) die($err_msg);
  }else{
